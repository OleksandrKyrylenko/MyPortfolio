 ...
 */
    ...
    (() => {
        ...
        const script_name = document.getElementById("name");
        const email = document.getElementById("email");
        const phone = document.getElementById("phone");
        const script_textarea = document.getElementById("textarea");
        script_form.addEventListener("submit", (e => {
            ...
            if (isFormValid) {
                const formData = {
                    name: script_name.value.trim(),
                    email: email.value.trim(),
                    phone: phone.value.trim(),
                    textarea: script_textarea.value.trim()
                };
                sendMessage(formData);
                script_form.reset();
                setTimeout((() => {
                    if (modules_flsModules.popup) {
                        const popup = script_form.dataset.popupMessage;
        ...
        const setError = (element, message) => {
            const inputControl = element.closest(".form__inner");
        ...
        const isValidEmail = email => /^\w+([\.-]?\w+)*@\w+([\.-]?\w+)*(\.\w+)*(\.\w{2,8})+$/.test(email);
        const validateInputs = () => {
            const userNameValue = script_name.value.trim();
            const emailValue = email.value.trim();
            const phoneValue = phone.value.trim();
            const textareaValue = script_textarea.value.trim();
            const tel = selector.inputmask.unmaskedvalue();
            let isFormValid = true;
            if (userNameValue === "") {
                setError(script_name, "Name is required");
                isFormValid = false;
            } else if (userNameValue.length === 1) {
                setError(script_name, "Invalid name");
                isFormValid = false;
            } else setSuccess(script_name);
            if (emailValue === "") {
            ...
            if (textareaValue === "") {
                setError(script_textarea, "Enter your message.");
                isFormValid = false;
            } else setSuccess(script_textarea);
            return isFormValid;
        };
        ...
        for (let i = 0; i < totalNavList; i++) {
            ...
            a.addEventListener("click", addClassItem);
        }
        function addClassItem() {
            for (let i = 0; i < totalSection; i++) allSection[i].classList.remove("back-section");
            for (let j = 0; j < totalNavList; j++) {
                if (navList[j].querySelector("a").classList.contains("active")) allSection[j].classList.add("back-section");
                navList[j].querySelector("a").classList.remove("active");
            }
            this.classList.add("active");
            showSection(this);
            document.documentElement.classList.contains("menu-open") ? script_menuClose() : null;
        }
